...
_ecore_evas_gix_move(Ecore_Evas *ee, int x, int y)
{
  if ((x != ee->x) || (y != ee->y)){
   ...
   printf("%s() got line %d OK, set %dx%d\n",__func__,__LINE__,x,y);
}
				     ...
				     int x, int y, int w, int h)
{
   ...
   if (parent != 0)
     ...
     ee->prop.window = ecore_gix_window_new(GI_DESKTOP_WINDOW_ID,0, x, y, w, h);
...
//        putenv((char*)"DESKTOP_STARTUP_ID=");
  ...
  // ecore_x_input_multi_select(ee->prop.window);
   ...
   ERR("%s() window id %d, %dx%d OK.",__func__, ee->prop.window,w,h);
   return ee;
}
